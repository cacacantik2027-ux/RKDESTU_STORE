...
// --- notifikasi ke Telegram admin (chat pribadi) ---
// PENTING: kalau ada FOTO, kirim FOTO ITU SENDIRI ke admin (sebelumnya
// bug: cuma teks "Mengirim foto" yang dikirim, foto aslinya TIDAK
// PERNAH diteruskan ke Telegram admin sama sekali).
if ($imageFilename) {
    $photoCaption = "💬 <b>Live Chat — Website</b>\n" .
                    "👤 " . htmlspecialchars($name, ENT_QUOTES) .
                    ($tgUsername !== '' ? " (@{$tgUsername})" : '') . "\n" .
                    ($page !== '' ? "📄 {$page}\n" : '') .
                    ($text !== '' ? "\n" . htmlspecialchars($text, ENT_QUOTES) : '') .
                    "\n\n<i>Balas lewat Panel Admin di website, bukan di sini.</i>";
    rk_tg_send_photo(defined('TELEGRAM_ADMIN_CHAT_ID') ? TELEGRAM_ADMIN_CHAT_ID : '', CHAT_UPLOAD_DIR . '/' . $imageFilename, $photoCaption);
} else {
...
// --- deteksi otomatis bukti transfer (foto + sinyal nominal/kata transfer) ---
// CATATAN PERBAIKAN: sebelumnya WAJIB caption foto itu sendiri menyebut
// PERSIS nama toko + nominal dalam SATU pesan yang sama -> di praktiknya
// pengunjung sering ketik nominalnya di pesan TEKS terpisah lalu upload
// foto tanpa caption apa-apa, jadi TIDAK PERNAH terdeteksi. Sekarang kita
// ikutkan beberapa pesan TEKS terakhir di sesi yang sama sebagai konteks,
// dan tidak lagi mewajibkan nama toko disebut ulang (wajar pengunjung
// tidak mengulang nama toko saat sedang chat di website toko itu sendiri).
$proofDetected = false;
if ($imageFilename) {
    $recentContext = rk_chat_recent_text($session, 6);
    if (rk_detect_payment_proof($text, $recentContext)) {
        $proofDetected = true;
